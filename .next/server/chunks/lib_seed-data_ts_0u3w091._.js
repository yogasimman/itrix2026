module.exports=[48020,e=>{"use strict";var i=e.i(62294);let t=[{id:1,name:"Micro SD Card (32GB)",description:"32GB Micro SD Card for data storage",pinout:"Standard SD interface - VCC, GND, MISO, MOSI, SCK, CS",category:"Storage",quantity:10,code_snippet:`// Micro SD Card with Arduino
#include <SPI.h>
#include <SD.h>
...
// No Arduino code required - plug into USB port`},{id:3,name:"Small Servo Motor",description:"9g micro servo motor for precise angular control",pinout:"Orange: Signal (PWM), Red: VCC (5V), Brown: GND",category:"Actuator",quantity:10,code_snippet:`// Small Servo Motor Control
#include <Servo.h>
...
}`},{id:9,name:"LED (Bright White)",description:"High brightness white LED",pinout:"Long leg: Anode (+), Short leg: Cathode (-). Use 220 ohm resistor.",category:"Output",quantity:50,code_snippet:`// White LED Blink
const int ledPin = 9;
...
void loop() {
  ...
  digitalWrite(ledPin, LOW);
  delay(500);
}`},{id:10,name:"LED (Red)",description:"Standard red LED",pinout:"Long leg: Anode (+), Short leg: Cathode (-). Use 220 ohm resistor.",category:"Output",quantity:50,code_snippet:`// Red LED with PWM Fade
const int ledPin = 9;
...
}`},{id:11,name:"LED (Green)",description:"Standard green LED",pinout:"Long leg: Anode (+), Short leg: Cathode (-). Use 220 ohm resistor.",category:"Output",quantity:50,code_snippet:`// Green LED Status Indicator
...
void loop() {
  ...
  digitalWrite(greenLed, LOW);
  delay(500);
}`},{id:12,name:"LED (Yellow)",description:"Standard yellow LED",pinout:"Long leg: Anode (+), Short leg: Cathode (-). Use 220 ohm resistor.",category:"Output",quantity:50,code_snippet:`// Yellow LED Warning Signal
...
// Typical usage: Between VCC and GND near ICs`},{id:15,name:"Capacitors 100uF",description:"100 microfarad electrolytic capacitor",pinout:"Polarized - Long leg/marked stripe side: Positive (+), Short leg: Negative (-)",category:"Electronic",quantity:50,code_snippet:`// Electrolytic Capacitor Usage Notes
...
// No direct Arduino code - hardware component
// Connect: Positive to VCC, Negative to GND`},{id:16,name:"Capacitors 100pF",description:"100 picofarad ceramic capacitor",pinout:"Non-polarized - can be connected either way",category:"Electronic",quantity:50,code_snippet:`// Small Capacitor Usage Notes
// 100pF capacitors are used for:
// - High frequency filtering
// - RF applications
// - Crystal oscillator circuits

// No direct Arduino code - hardware component`},{id:17,name:"Diodes IN4007",description:"1N4007 rectifier diode for protection",pinout:"Band/stripe side: Cathode (-), Other side: Anode (+)",category:"Electronic",quantity:50,code_snippet:`// Diode Usage for Motor Protection
...
// No direct Arduino code - hardware protection component`},{id:18,name:"Resistors 220 ohm",description:"220 ohm resistor for LED current limiting",pinout:"No polarity - can be connected either way",category:"Electronic",quantity:50,code_snippet:`// 220 ohm Resistor - LED Current Limiting
// For standard LEDs with 5V supply:
// I = (5V - 2V) / 220 ohm = 13.6mA (safe for LEDs)

// Connect in series with LED:
// Arduino Pin -> 220 ohm -> LED Anode -> LED Cathode -> GND`},{id:19,name:"Resistors 560 ohm",description:"560 ohm resistor",pinout:"No polarity - can be connected either way",category:"Electronic",quantity:115,code_snippet:`// 560 ohm Resistor Usage
// Typical uses:
// - LED current limiting (lower current than 220 ohm)
// - Voltage dividers
// - Signal conditioning`},{id:20,name:"Resistors 1 kilohm",description:"1K ohm resistor",pinout:"No polarity - can be connected either way",category:"Electronic",quantity:50,code_snippet:`// 1K ohm Resistor - Transistor Base Limiting
// For NPN transistor switching:
// Arduino Pin -> 1K ohm -> Transistor Base

// Also used for pull-down resistors and voltage dividers`},{id:21,name:"Resistors 4.7 kilohm",description:"4.7K ohm resistor",pinout:"No polarity - can be connected either way",category:"Electronic",quantity:50,code_snippet:`// 4.7K ohm Resistor Usage
// Common uses:
// - I2C pull-up resistors
// - Voltage dividers
// - Sensor interfaces`},{id:22,name:"Resistors 10 kilohm",description:"10K ohm resistor",pinout:"No polarity - can be connected either way",category:"Electronic",quantity:50,code_snippet:`// 10K ohm Resistor - LDR Voltage Divider
// Circuit: 5V -> LDR -> A0 -> 10K ohm -> GND
// This creates a voltage divider for light sensing

const int sensorPin = A0;
int lightLevel = analogRead(sensorPin);`},{id:23,name:"Resistors 1 megohm",description:"1M ohm resistor",pinout:"No polarity - can be connected either way",category:"Electronic",quantity:50,code_snippet:`// 1M ohm Resistor Usage
// High resistance applications:
// - Touch sensors
// - High impedance inputs
// - Timing circuits with capacitors`},{id:24,name:"Resistors 10 megohm",description:"10M ohm resistor",pinout:"No polarity - can be connected either way",category:"Electronic",quantity:50,code_snippet:`// 10M ohm Resistor Usage
// Very high resistance applications:
// - Electret microphone biasing
// - High impedance buffers
// - Capacitive touch sensing`},{id:25,name:"Analog to Digital Converter (MCP3204)",description:"12-bit ADC with SPI interface",pinout:"VDD: 5V, VREF: Reference Voltage, AGND: Analog GND, DGND: Digital GND, CLK: SPI Clock, DOUT: Data Out, DIN: Data In, CS: Chip Select, CH0-CH3: Analog Inputs",category:"Interface",quantity:20,code_snippet:`// MCP3204 12-bit ADC
#include <SPI.h>
...
// Example UID: 0x04 0xA3 0x2B 0x1C`},{id:39,name:"RFID Reader",description:"RC522 RFID reader module",pinout:"SDA: SPI SS, SCK: SPI Clock, MOSI: SPI MOSI, MISO: SPI MISO, IRQ: Interrupt, GND: Ground, RST: Reset, 3.3V: Power",category:"Identification",quantity:5,code_snippet:`// RFID RC522 Reader
#include <SPI.h>
#include <MFRC522.h>
...
// Use TI BLE Stack for programming`},{id:43,name:"TI Simplelink Sensor DevPack",description:"Sensor expansion pack for TI Simplelink",pinout:"Connects to Simplelink boards via headers",category:"Development Kit",quantity:1,code_snippet:`// TI Simplelink Sensor DevPack
// Extension for Simplelink boards
// Adds additional sensor capabilities

// Programmed via Code Composer Studio`},{id:44,name:"Zigbee Wifi VXBB-02",description:"Zigbee WiFi bridge module",pinout:"VCC, GND, TX, RX, GPIO pins",category:"Communication",quantity:1,code_snippet:`// Zigbee VXBB-02 Module
...
void loop() {
  ...
  delay(1000);
}`}],o=[{id:1,title:"The Midnight Intruder Alarm",team_number:1,situation:'The college lab is locked after 6 PM - but someone has been sneaking in undetected. The warden installs a lock, but it gets bypassed. CCTV is expensive. The warden reaches out to the IoT lab team and says: "I need something that detects motion in the dark and raises an alarm - but lets me silence it quickly if it\'s just me checking in."',what_to_build:"1. Read LDR via analogRead to determine dark/light. When dark, monitor PIR for motion.\n2. Motion detected -> red LED flashes rapidly + piezo alarms.\n3. Pressing the pushbutton within 5 seconds silences the alarm.\n4. If button is NOT pressed within 5 s, alarm continues for a full 20 seconds.\n5. After alarm ends, system re-arms automatically. Use millis() - no delay().",components:["Photo Resistor LDR","PIR Motion Sensor","LED (Red)","Piezo Capsule","Pushbuttons"]},{id:2,title:"The Touchless Hospital Dustbin",team_number:2,situation:'In the college medical room, bins are constantly being touched by multiple people - spreading germs during flu season. A nursing student raises a concern: "We need bins that open themselves when you bring your hand close. No touch, no infection." The challenge is to make the lid movement smooth and controlled - not a sudden snap.',what_to_build:"1. Poll the HC-SR04 every 150 ms. When a hand is detected within 15 cm, smoothly sweep the servo from 0 degrees (closed) to 90 degrees (open) - one degree at a time, 8 ms between steps.\n2. Hold open for 4 seconds, then sweep back to 0 degrees at the same speed.\n3. Green LED stays on while lid is open.\n4. One short beep when opening, two short beeps when closing.\n5. Block re-trigger while lid is already moving or open.",components:["Ultrasonic Sensor","Small Servo Motor","LED (Green)","Piezo Capsule","Resistors 220 ohm"]},{id:3,title:"The Seminar Hall Occupancy Counter",team_number:3,situation:'The college seminar hall seats 80 people but is never managed properly. Some events are dangerously overcrowded while others are half-empty. The Events Committee asks: "Can we have a live headcount at the door - something that tells us exactly how many people are inside right now, without cameras or manual counting?"',what_to_build:"1. Mount IR sensor A and B 5 cm apart at the door frame.\n2. Track which sensor triggers first: A then B = entry (count++), B then A = exit (count--).\n3. Allow a 300 ms window to detect the second sensor after the first fires.\n4. Display occupancy on 3 LEDs: green = low (0-3), yellow = medium (4-7), red = high (8+).\n5. A short beep on each valid entry or exit. Count must not go below 0.\n6. Log direction and count to Serial on every change.",components:["IR Sensor","IR Sensor","LED (Green)","LED (Yellow)","LED (Red)"]},{id:4,title:"The Smart Staircase Lighting",team_number:4,situation:"The hostel staircase is completely dark after 10 PM. A student broke their wrist tripping in the dark last semester - the switch is at the wrong end and nobody bothers to flick it. The hostel warden wants lights that respond automatically, stay on long enough for safe passage, and need no switches at all.",what_to_build:"1. Two PIR sensors simulate top and bottom of a staircase.\n2. PIR-top triggers -> green LED on for 20 s; resets timer if motion is detected again within the window.\n3. PIR-bottom triggers -> red LED on for 20 s with the same re-trigger logic.\n4. Both LEDs can be on simultaneously - they operate fully independently.\n5. No blocking code - use millis() for both independent 20 s timers.\n6. Serial-print which PIR triggered and each LED's remaining time every 5 s.",components:["PIR Motion Sensor","PIR Motion Sensor","LED (Green)","LED (Red)","Resistors 220 ohm"]},{id:5,title:"The Smart Pedestrian Crossing",team_number:5,situation:"A narrow lane through the college campus has become dangerous. Vehicles speed through while pedestrians step out from between parked cars. Two near-misses in the same month forced the administration to act. A proper traffic light system would take months of approval. You have a weekend and five components.",what_to_build:"1. Idle state: red LED on (stop), servo at 0 degrees (barrier down).\n2. Pedestrian presses button -> check HC-SR04 for vehicle (distance < 40 cm = vehicle present).\n3. Vehicle present -> red LED flashes fast, deny crossing, recheck every 500 ms for up to 10 s.\n4. No vehicle -> green LED on + servo sweeps to 90 degrees (barrier up). Hold crossing open for 7 s.\n5. Servo sweeps back to 0 degrees, green off, red on.\n6. Serial-log each button press, vehicle detection result, and crossing duration.",components:["Pushbuttons","Ultrasonic Sensor","Small Servo Motor","LED (Green)","LED (Red)"]},{id:6,title:"The Malfunctioning Traffic Signal",team_number:6,situation:"The traffic signal at the college entrance has broken down. It's stuck on red and hasn't changed in two days. Vehicles are piling up and pedestrians are jaywalking dangerously. The college principal calls the IoT lab team: \"I need a temporary working traffic signal installed by tomorrow morning - one that runs automatically but lets a traffic officer override it manually with a button.\"",what_to_build:"1. Normal operation: cycle automatically - white LED on (go) for 8 s -> yellow LED blink 3 times (slow down) -> relay clicks off (stop/red, simulated) -> repeat.\n2. Piezo beeps once at every phase change so people nearby can hear the signal switch.\n3. Pushbutton = officer override: immediately jumps to stop phase (relay OFF, yellow LED off, white LED off) and holds for 15 s before resuming auto-cycle.\n4. During override: yellow LED flashes rapidly to indicate manual control is active.\n5. Use millis() for all phase timing - no delay(). Serial-print current phase and time remaining every second.",components:["LED (Yellow)","LED (Bright White)","Pushbuttons","Piezo Capsule","Relay Module"]},{id:7,title:"The Forgotten Classroom Lights",team_number:7,situation:'Every evening the college classrooms are left with lights blazing - the last student out never bothers to switch them off. The electricity bill has doubled. The facilities manager is furious: "I want the lights to turn off automatically when it gets dark outside and nobody is in the room - but I still want a manual override button for when someone genuinely needs to keep them on."',what_to_build:"1. Read LDR every 1 s using millis(). When ambient light drops below threshold (< 300 ADC) -> relay ON, simulating room lights turning off (or fan stopping).\n2. When light is above threshold (bright, daytime) -> relay stays OFF (lights not needed).\n3. White LED mirrors relay state - on when relay is on.\n4. Yellow LED = system status indicator, always on to show the controller is active.\n5. Pushbutton toggles a manual override flag: when override is ON, relay is forced OFF regardless of LDR (lights stay on). Press again to release override.\n6. Serial-print LDR value, relay state, and override flag every 2 s.",components:["Photo Resistor LDR","Relay Module","LED (Yellow)","LED (Bright White)","Pushbuttons"]},{id:8,title:"The Runaway Workshop Fan",team_number:8,situation:"The college workshop bench fan has no speed limit. Students routinely crank it to maximum, and last semester the plastic blades cracked under stress, sending a fragment across the room. Nobody was hurt - barely. The HOD has mandated: the fan must not exceed a safe speed, and must slow itself down automatically if a student tries to go too fast.",what_to_build:"1. Read potentiometer every 200 ms. Map ADC (0-1023) to PWM (0-255) and drive DC motor via NPN transistor.\n2. Yellow LED = slow zone (PWM < 100). Both LEDs off = medium (100-200). Red LED = fast (> 200).\n3. When PWM > 220 (danger zone): automatically cap PWM at 180 - override the potentiometer input.\n4. Red LED rapid flash + piezo 3000 Hz warning beep every 2 s during override.\n5. When pot is dialled back below 200, restore normal pot control and clear override.\n6. Serial-log pot value, PWM applied, speed zone, and override flag every 500 ms.",components:["Potentiometer 10 kilohm","DC Motor 5V","Transistor","LED (Yellow)","LED (Red)"]}];e.s(["seedDatabase",0,function(){if((0,i.initializeDatabase)(),(0,i.getAllComponents)().length>0)return void console.log("Database already seeded");for(let e of t)(0,i.addComponent)(e);for(let e of o){(0,i.addScenario)({id:e.id,title:e.title,situation:e.situation,what_to_build:e.what_to_build,team_number:e.team_number});let o=e.components.map(e=>t.find(i=>i.name===e)?.id).filter(e=>void 0!==e);(0,i.setScenarioComponents)(e.id,o)}console.log("Database seeded successfully")}])}];

//# sourceMappingURL=lib_seed-data_ts_0u3w091._.js.map